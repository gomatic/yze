...
func TestRegistrationsCatalog(t *testing.T) {
	...
	assert.Equal(t, []string{"yze/go/anonstruct", "yze/go/boolname", "yze/go/emptyiface", "yze/go/errconst", "yze/go/gotostmt", "yze/go/namedtypes", "yze/go/ptrparam", "yze/go/ptrrecv"}, ruleIDs(regs))
	for _, r := range regs {
...
func TestFilterByGroupKeepsMatching(t *testing.T) {
	assert.Len(t, yze.Filter(yze.Registrations(), "go", nil), 8)
...
func TestFilterWithNoConstraintsKeepsAll(t *testing.T) {
	assert.Len(t, yze.Filter(yze.Registrations(), "", nil), 8)
}
